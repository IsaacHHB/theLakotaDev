...
export default function Edit({ auth, invites, events, event_id }) {
    ...
    useEffect(() => {
        ...
        setData('invites', invites.invites || []);
    }, [event_id, invites]);
    ...
    const addInvite = () => {
        const newInvites = [...data.invites, {
            id: null, // Ensure new invites have no ID
            name: '',
            email: '',
            plus_one: false,
            plus_one_name: '',
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        put(route('event-invite.update', data.event_id), {
            preserveScroll: true,
            onSuccess: () => reset(),
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-300 leading-tight">Edit Invites</h2>}
        >
            <Head title="Edit Invites" />
            <div className="py-12">
